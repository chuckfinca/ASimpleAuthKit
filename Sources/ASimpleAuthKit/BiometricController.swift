...
public class BiometricController: ObservableObject {
    ...
    /// This will test biometric authentication to ensure it works before enabling
    ...
    /// Check if biometric authentication should be required for app launch
    /// Call this during app startup to determine if user should be prompted for biometrics
    public func shouldRequireBiometricsForCurrentSession() async -> Bool {
        ...
        // Check if there's a current Firebase user session
    ...
    /// Require biometric authentication for the current user session
    /// Call this when you want to prompt the user for biometric authentication
    ...
    /// This should be called after biometric auth succeeds to update preferences
    ...
    /// Clear all biometric preferences (useful for app reset or user switching)
    ...
    /// Complete biometric setup flow - enables biometrics and sets up user association
